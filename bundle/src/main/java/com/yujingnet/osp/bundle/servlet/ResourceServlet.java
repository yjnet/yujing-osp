...
//			marshaller.setProperty("com.sun.xml.bind.xmlDeclaration", Boolean.FALSE);
		...
		for (int i=0; i < paths.length; i++) {
			...
			if (isDiv(hashIndex)) {
				if (!rootDiv.containsKey(path.toString())) {
					...
					// set <span>
					Span span = new Span();
					Resource res = resourceResolver.getResource(path.toString());
					String type = res.getResourceType();
					template = getTemplateName(res, template);
	...
	private String getTemplateName(Resource res, String defaultValue){
		try {
			if (res != null && ContentNodeType.YJ_PAGE.equals(res.adaptTo(Node.class).getPrimaryNodeType().getName())) {
				Resource jcrContent = res.getChild("jcr:content");
				if (jcrContent != null) {
					return jcrContent.getValueMap().get(ContentNodeType.YJ_TEMPLATE, defaultValue);
				}
			}
